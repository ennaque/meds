...
class SearchResultCombiner implements SearchResultCombinerInterface
{
     ...
     * @return array<int>
     */
    private function findNeedleEntries(string $content, string $needle,
                                       int $count): array
    {
        $result = array();
        $offset = 0;
     ...
     * @return array<string>
     */
    private function extractNeedleEntries(string $content, string $needle,
                                          array $entriesPositions): array
    {
        ...
        for ($i = 0; $i < count($entriesPositions); ++$i)
        {
            ...
            if($i == 0)
            {
                if($entriesPositions[$i] >= $cutLength)
                {
                    $from = $entriesPositions[$i] - $cutLength;
                    $length = $cutLength;
                }
                else
                {
                    $from = 0;
                    $length = $entriesPositions[$i];
                }
            }
            else
            {
                $previousPosition = $entriesPositions[$i - 1] + strlen($needle);
                $distance = $entriesPositions[$i] - $previousPosition;
                ...
                {
                    $from = $entriesPositions[$i] - $distance;
                    $length = $distance;
                }
            }
            $result[$ri] .= substr($content,
                                   $from,
                                   $length);
            ...
            {
                if(($entriesPositions[$i+1] - $entriesPositions[$i] + strlen($needle)) >= ($cutLength*2))
                {// if the next entry is far enough (double distance from current one), ...
                    $result[$ri] .= substr($content, // ...then copy $cutLength symbols after the needle to the result ...
                                           $entriesPositions[$i] + strlen($needle),
                                           $cutLength);
                    $ri += 1; // ...and go over towards the next result string
                    $result[$ri] = "";
                }
                // otherwise -- do nothing: the next needle entry will be added to the current result string in the next loop iteration
            }
            else
            {// the last needle entry
                $length = 0;
                if((strlen($content) - $entriesPositions[$i] + strlen($needle)) >= $cutLength)
                    $length = $cutLength;
                else
                    $length = strlen($content) - $entriesPositions[$i] + strlen($needle);
                $result[$ri] .= substr($content,
                                       $entriesPositions[$i] + strlen($needle),
                                       $length);
            }
        ...
        for($i = 0; $i < count($result); ++$i)
        {
            ...
            $lastChar = $result[$i][strlen($result[$i])-1];
            $from = 0;
            $to = 0;
            if($firstChar != ' ')
                $from = strpos($result[$i], ' ', 1) + 1;
            else
                $from = 1;
            if($lastChar != ' ' && $lastChar != '.' && $lastChar != ',' && $lastChar != ';' && $lastChar != ':')
                $to = strrpos($result[$i], ' ', -1);
            else
                $to = strlen($result[$i]) - 1;
            $result[$i] = ":::" . substr($result[$i], $from, $to - $from) . ":::";
        }
        return $result;
    }
    ...
    public function letsTestSomeShit(string $content, string $needle, int $count)
    {
        ...
        for($i = 0; $i < count($extractionResult); ++$i)
            echo("\n$i: $extractionResult[$i]");
...
echo("This shit out is started!\n\n");

$guineaPig = new SearchResultCombiner;
$guineaPig->letsTestSomeShit($testContent, "PHP", 100);

echo("\nThis shit out is stopped!");
?>